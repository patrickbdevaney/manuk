...
| **`navigator.userAgentData` (Client Hints)** + canonical UA/`platform` | modern UA-sniff reads us as stale/fake; UA contains `"Manuk"`, `platform` is non-canonical `"linux x86_64"` → LinkedIn degraded path | **works** (t286 userAgentData; t458 `deviceMemory`+canonical `platform`→`"Linux x86_64"`, `G_DEVICE_IDENTITY`) | UA-string *policy* (whether to advertise a recognized Chrome UA) is the only residual identity decision; the CH surface, `deviceMemory` and legacy `platform` are now canonical and self-consistent. |
