...
| **An inline element with a frame on the INLINE AXIS ONLY** | `<code>`, `<kbd>`, padded `<a>` chips and pills, syntax-highlighted spans, badge spans — the horizontal-only frame is the overwhelmingly common inline decoration, and it is exactly the shape that breaks | ⚠ MEASURED, NOT YET FIXED (tick 993) — t851 established that a non-replaced inline's box is its own content area **resolved PER AXIS**, and that holds for a plain inline and for one with an ALL-SIDES border (`border:12px` reads `[10,-10,34,43]`, exact). But `border-left:12px` and `padding-left:12px` both read `[10,0,22,21]` against Chrome's `[10,2,22,19]`: **the inline axis is right in every row** (the frame advances the pen correctly) and only the BLOCK axis goes wrong, and only when the VERTICAL frame is zero — the box top becomes the LINE BOX's top rather than the content area's, 2px of half-leading high and 2px over in height. A narrow path, not the general case, which is why t851's fix did not cover it. Fixture `/tmp/ib.html` |
