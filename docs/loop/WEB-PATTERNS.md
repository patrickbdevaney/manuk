...
| **`justify-self` on a grid item** | a grid item aligned to the end or centre of its own track: form labels against their fields, card action rows, right-aligned numerics in a data grid. Priced at **1.8%** of the burndown corpus | ✅ **(tick 980, `G_SELF_ALIGNMENT`)** — `justify-self: end` put a 60px item at **x=0 inside a 200px track where Chrome puts it at 140**. ⚠ **The property was absent at all three layers while its axis-twin was complete at all three**: no `ComputedStyle` field, no parse arm, no Stylo recovery, and `taffy_tree.rs` carried `align_self` with no partner line. *A gap survives when the neighbouring line looks like coverage.* `align-self` was correct in BOTH flex and grid throughout — 5/5 in the same probe — and that control is what identified the defect. Found by surface audit #38 pricing twelve Safari 26.x features on the corpus: ten came back at ≤0.6%, `align-self` (8.2%) was the only one worth probing, and it brought its twin with it. RED-proven, and the `justify-self` row is the ONLY one that moves. |\n| **`transform-origin`** | any transform that is not about the box centre: flip cards, accordions, zoom-on-hover tiles, origin-anchored scale/rotate in every design system. `transform:` is 34.5% of the corpus | ✅ **(tick 976, `G_TRANSFORM_3D`)** — **unimplemented while looking implemented**: `resolve_transform` has taken an `origin` parameter since it was written and all THREE call sites passed the hard-coded box centre. ⚠ *A defaulted parameter that no caller ever overrides is indistinguishable from an unimplemented property until something measures it* — grep finds the name, the doc explains the semantics, the behaviour is a constant. `scale(2)` on a 100px box: `origin:0 0` Chrome [0,220] vs ours [-50,200]; `100% 100%` Chrome [-100,810] vs ours [-50,830]. New non-inherited `(Dim, Dim)` property, parsed in MinimalCascade and recovered from Stylo. 21/22 of the transform battery now Chrome-exact. ⚠ `top`/`bottom` name the Y axis WHEREVER they appear (`top left` == `left top`); our parser handles it and the gate does NOT prove it — Stylo resolves keywords on the shipping path, and the assertion says so rather than claiming a proof it lacks. |\n| **`translate3d()` and the 3D transform spellings** | every animation library, carousel, drawer, sticky header and modal on the modern web: `translate3d(x,y,0)` is THE idiom for putting an element on its own compositor layer. `transform:` is on **34.5%** of the burndown corpus | ✅ **(tick 975, `G_TRANSFORM_3D`)** — `Translate3D`/`Scale3D`/`Rotate3D`/`Matrix3D` all fell into `stylo_map.rs`'s `_ => {}` and were **silently dropped**, leaving the element at its **untransformed position** — the largest error the property can produce (`translate3d(20px,10px,0)`: Chrome x=20, ours x=0). ⚠ **The comment above the catch-all was the alibi**: *"3D/perspective skipped — our paint model is 2D"* is true of a genuine 3D effect and false of `translate3d(x,y,0)`, which has no 3D component; and `rotateZ` was mapped all along, so the family looked handled. The 2D projection is EXACT (with no `perspective`, `z` contributes nothing on screen); `rotate3d` is taken only about the **z** axis, RED-proven by an X-axis row that must stay 100×40. ⚠ The first fix went to `parse_transform` in `lib.rs` and changed nothing — that is the MinimalCascade path and the shipping cascade is Stylo; both are fixed so the fallback agrees. Named, measured, not built: **`transform-origin` is unimplemented** (we always transform about the centre). |\n| **An unstyled form control's UA box** | `<button>` is **55.6%** of the burndown corpus and `<input>` **51.5%** — the two commonest constructs measured; every page with a form, a search box or a call-to-action | ✅ **button border (tick 972, `G_UA_CONTROL_METRICS`)** — Chrome's UA computes `border: 2px outset` on a `<button>` and we declared **1px**, so every button on every page was **2px short in BOTH axes** (58.2 × 21 in Chrome against our 56 × 19). Read out of Chrome with `getComputedStyle`, not guessed. ⚠ **The same measurement refuted surface audit #37's claim that we inherit the body's font-size for controls**: the UA rule (`13.333px Arial`) has always been there, and our unstyled `<select>` (19) and `<textarea>` (182 × 36) are **exactly** Chrome's — a font-size error could not leave two controls exact. Both rows are now pinned in the gate. **Two residues refused as TRADES and pinned at their wrong values**: a text `<input>` is 19 against 21 but its width is already exactly 205 (a `size`-driven formula with its own intercept), and a 4-row `<select multiple>` is 72 against 70 but zeroing our select's vertical padding would take the single-line dropdown from an exact 19 to 17. |\n| **An inline `<svg>`'s line box** — the icon, everywhere | nav bars, toolbars, chips, buttons, social rows, every icon on the modern web: inline `<svg>` is on **34.5% of the burndown corpus**, the sixth most common construct measured | ✅ **(tick 967, `G_REPLACED_BASELINE`)** — an inline `<svg>` made its line box **30px where Chrome gives 20** and sat **14px down inside it**, while an `<img>` of the identical 16×16 size was correct in both engines on the same line. The svg's box reported a baseline of **0** — its own TOP — so all 16px hung BELOW the baseline: `strut ascent 14 + 16 = 30`. **The rule was right and its DOMAIN was wrong**: CSS 2.1 §10.8.1's last-in-flow-line-box baseline was applied to EVERY atomic inline, and a replaced element has no in-flow line boxes by definition — asking one is asking our internal box structure a question the spec never asks it, and `<svg>` answers because unlike `<img>` it has element children. **`<img>` was right all along by accident.** The guard is a NARROWING and the gate pins it both ways: widen it to every atomic and a text-bearing `inline-block` goes to 22 against Chrome's 19.19. A line box 10px too tall drags everything below it down the page. Residue, measured and named: `<div><button><svg/></button></div>` is still 32 against Chrome's 24 — the same shape one level up, an inline-block's baseline when its last line box holds a vertically-aligned replaced item.  **t970 EXTENDS IT to every inline-block that CONTAINS a replaced element** — the icon-BUTTON shape, 23.4% of the corpus: `<span inline-block><svg/></span>` was **34 against Chrome's 20** and the `<img>` version **24**, two symptoms of one bug. Atomics are filed as sibling boxes, so §10.8.1's search walks them: `<svg>` had a subtree so the search descended and believed a `<rect>` fragment at the box's own TOP (baseline 0); `<img>` had none so the search returned `None` and the caller took the *no-in-flow-line-boxes* fallback — **taken on a line box that exists**. A replaced kid contributes its OWN bottom edge, and both land on 20. ⚠ *Skipping* the subtree only reaches 24 (RED-proven) — the 4px between skip and edge is the rule. `#end` over eight rows: 232 → 179 against Chrome's 186. Residue, named: the `<button>` rows are 24 against 26, which is the pre-existing UA control-height −2 that the text-only button shares.|
| **A `<select>` is as wide as its WIDEST option** | approximately every form on the web: country/state pickers, currency and timezone selects, filter and sort dropdowns — any control whose rendered option is shorter than one it must hold | ✅ **(tick 964, `G_SELECT_WIDTH`)** — the control renders ONE option and RESERVES room for all of them, and we sized every select to the option it happened to be displaying. ⚠ **This was never a multi-select bug: the ordinary dropdown measured 62 against Chrome's 76** on the gate fixture, so a picker showing a short entry clipped every longer one. Chrome-measured rule, one line: **`widest option + 6`, plus a 17px arrow strip when it is a dropdown** — independent of the row count and of whether the list scrolls (5 options in 4 rows measures the same 59.36 as 5 in 10, so ⚠ **the ~6px scrollbar term t963 predicted does not exist**; it was an artefact of comparing against the rendered option instead of the widest). The engine's missing term is exactly `widest − shown`, a RESERVE on top of the extent that already contains the rendered option. **Both halves had to land together** and the gate carries both directions: dropping the arrow alone triples the width error (44.2px → 81.4px), keeping it for a list box gives 76.36 against 59.36. `appearance:none` removes the ARROW, never the reserve — otherwise every restyled design-system select clips its own entries. Nine of ten controls now Chrome-exact; RED-proven five ways. Named residue: `<select multiple size=1>` is 95 in Chrome where every formula predicts ~62, deliberately not modelled. |
| **`<select multiple>` / `<select size=N>`** — the LIST BOX | every filter sidebar, admin form, faceted search and multi-value picker on the web: a multi-select is a *sized scrolling list box*, not a dropdown | ✅ **height (tick 963, `G_SELECT_LISTBOX`)** — there was **no branch for either attribute anywhere**, so every list box rendered **one line tall** and, because a control's height displaces everything after it, the content below ten controls landed **288px too high** (313 against Chrome's 601.6). That is the burndown's dominant `dy` term arriving through a form control, on the form-heavy pages the CrUX tail is full of. HTML's *display size* — `size` when ≥1, else 4 for `multiple`, else 1 — with the list-box model applying only above **one** row (Chrome-measured: `multiple size=1` is the dropdown's 21, not a one-row box's 22.2). **Content height = `rows × (1.2 × font-size + 1)`**, fitted against Chrome at six font sizes to ≤0.03px — ⚠ it is **not** the font's line box (18 vs 20.2 at 16px), is font-family independent, and ignores `line-height`, so a fix derived from our text metrics is wrong at every size but the one it was written for. An explicit `height` still wins. RED-proven four ways. **Residue, named rather than bundled: the WIDTH.** Dropping the 17px dropdown-arrow strip — which a list box does not have — is a **measured regression in isolation** (total width error 44.2px → 81.4px): our width comes from the *selected* option and Chrome's from the *widest*, so the arrow was compensating for the wrong measurement. Also unfixed: rendering all N option rows (content, not sizing), and Chrome's UA ~13.333px `<select>` font, which we inherit instead. |
| **A preserved TAB and `tab-size`** — tab-indented preformatted text | every `<pre>` that indents with tabs: documentation code samples, config-file listings, `git diff` views, log panes, anything pasted from an editor that indents with tabs; and `<textarea>` (pre-wrap by UA default), where a tab is what the user typed | ✅ **(tick 962, `G_TAB_STOP` — `engine/page/tests/g_tab_stop.rs`)** — a tab contributed **zero advance**: `ab\tcd` rendered as `abcd` and `a\tb\tc\td` measured **31px against Chrome's 240.8, eight times too narrow**, so every tab-indented block lost its indentation structure entirely — and because the run came out narrower, **its wrap points moved**, making this a `dy` term on exactly the text-dense technical pages the corpus is full of. `tab-size` had no effect because there was nothing for it to scale. Now a `\t` in a `pre`/`pre-wrap` run is collected as its own `InlineItem::Tab` and placed by its own branch in `layout_inline`, where the pen exists: **the pen advances to the next multiple of `tab-size × the space advance` STRICTLY greater than where it is.** New inherited `TabSize::{Spaces(f32), Px(f32)}` (initial `Spaces(8)`), parsed in MinimalCascade under both `tab-size` and `-moz-tab-size` and recovered into the shipping Stylo cascade. Chrome-verified at four `tab-size` values plus a multi-tab row; RED-proven three ways. **Safety: a run with no tab in it produces exactly the one `Word` it always did.** Residue: `pre-line` collapses tabs (correct per spec); a tab is not modelled as its own soft-wrap opportunity inside `pre-wrap`. |
...
| pattern | where it shows up | status |
| --- | --- | --- |
| **A table given a `height` taller than its content — the equal-height/vertically-centred layout table, and every `<table height>` on the legacy web** | `display:table{height:N}` + `table-cell{vertical-align:middle}` is the pre-flexbox centring idiom; a `<table style="height:100%">` is how a decade of email-derived and CMS markup filled a pane. Also every wikipedia-class infobox, which burndown family #5 names as severe | ✅ (tick 933) — **t908 taught the table BOX to grow to its declared height and nothing inside it moved**, so the rows kept their natural heights and the declared height became **empty space at the bottom**: the box right, every row wrong, a `<td>` that should be 56 tall sitting at 26. ⚠⚠⚠ **FOUR GATES HAD ALREADY NAMED THIS ONE ALGORITHM, each from a different door, each pinned at OUR number with its mechanism written beside it** — `g_orphan_table_cell#c3` (t814, `[0 92 67x20]` vs Chrome `[0 90 300x80]`), `g_table_height_is_a_minimum#t10` (t908, unasserted, "genuinely the algorithm"), t925's border-spacing residue, `g_anonymous_table_row#mid` (t932, `400x24` vs `400x100`). **All four now assert Chrome, and the practice is the finding**: a residue pinned at our own number, with its mechanism named and an explicit instruction that a future fix must change the line deliberately, is what let four independent sightings converge instead of a fifth re-measurement. ⚠⚠⚠ **CSS 2.1 §17.5.3 declines to specify the distribution — *"implementation-dependent"* — so the spec cannot settle it and every number is a measurement of Chrome.** Two clauses, each with exactly one discriminating fixture row: **PROPORTIONAL to natural height, not equal shares** (two rows of natural 26 and 74 in 194 of usable height give **50.4 · 143.6**, where equal shares give 73 · 121 — the two models agree on every *equal-natural* row, so a fixture without a lopsided pair passes against the wrong algorithm), and **a row that SPECIFIES a height is EXCLUDED from the surplus rather than counted in it** (`height:100px` beside a natural-26 row gives **100 · 94**; proportional-over-both gives 154 · 40, and the RED proof prints 153.97, so it is not a rounding argument). ⚠ **A row's own `height` is a MINIMUM on its natural height** — the other half, unobservable on its own, which is why it is one tick and not two. Gated by `G_TABLE_ROW_HEIGHT_DISTRIBUTION` (13 claims, 2 CONTROLs), RED-proven three ways — delete the distribution (`#one_c` 56→26) · equal shares (`#uneq_a` 50.4→73) · include specified-height rows (`#spec_a` 100→153.97). ⚠ **The CONTROLs are t908's rule and they are what the plausible mis-phrasing breaks**: a fix written as *"make rows fill the table"* rather than *"distribute the SURPLUS"* shrinks a row whose content exceeds the declared height and passes everything else. ⚠ Left deliberately, with reasons: when EVERY row specifies a height the surplus stays at the bottom (Chrome **unmeasured** there — a non-answer, not a rule), and a row BOX is not inset by the horizontal border-spacing (Chrome reports a `<tr>` as 196 in a 200px table, we report 200 — a different quantity, moving no cell) |

| pattern | where it shows up | status |
| --- | --- | --- |
| **A typographic or icon WRAPPER — an inline element whose only child is another inline element, with a different `font-size`** | `<span class="h1"><span class="txt">…</span></span>`, `<a><span>label</span></a>` across font sizes, and every `<span class="icon"><i></i></span>` — the shape of nearly every icon, badge and typographic wrapper on the web | ✅ (tick 934 measured · tick 935 LANDED — all eight line boxes and their y positions byte-identical to Chrome, and the 12.8px cumulative drift below the wrapper is gone) — **CSS 2.1 §10.8 is unconditional: EVERY inline box contributes its leading to the line box, whether or not it DIRECTLY contains text.** Ours contributes only through the fragments its text produces, so a wrapper `<span>` with a larger `font-size` is invisible to the line box: `<span 24px><span 12px>nested</span></span>` in a `16px/1.5` div is **36 in Chrome and 24 here**, and with `line-height:normal` it is **28 vs 18**. ⚠⚠ **The discriminator is exact and it is what identifies the mechanism**: give the outer span ONE character of its own text (`<span 24><span 12>n</span>x</span>`) and we are correct at 36. A metrics, font or rounding error would move the five CONTROL rows too — this is **structural**. The line box height is folded over `LineFrag`s plus the strut (`layout/lib.rs:7545-7560`), `LineFrag`s come from `InlineItem`s, and `collect_inline_node` (`:6218`) emits an item only for text, atomics, spacers and breaks — **an element wrapping another element emits nothing**, so it never reaches the fold. FIX AS LANDED (t935): `InlineItem::Spacer` gained a `leading` field kept SEPARATE from `report_height` — the two were one number, and that conflation is exactly why a wrapper could not be expressed (a padded edge reports a tall rect and contributes ZERO leading per §10.6.1; a text-less wrapper is the mirror image, full leading and no rect at all). The carrier is emitted with `node: None` (metrics, never geometry) and `holds_line: false`. ⚠⚠⚠ **`ComputedStyle::line_height` IS NOT THE LINE HEIGHT** — it holds the raw `1.2 x font-size` fallback for `line-height:normal`, while `text_style` derives it from the FACE (`round(ascent+descent+lineGap)`): 19.2 vs 18 at 16px. The first draft read the raw field and `manuk-layout`'s button-centring test caught it on the first run. ⚠⚠ **A tolerance can make a gate unfalsifiable in the quantity it tests**: the RED proof for that wrong-field version came back GREEN, because 28.800003 against Chrome's 28 passes a 1.01 tolerance — the `normal` row alone now carries 0.5. ⚠ RESIDUE, pinned at OUR number: the line box is the right HEIGHT and the inner text is at the wrong BASELINE (Chrome 15px below the line top, ours 6) — t935 contributes `line_height` and NOT ascent/descent, because those are the metrics `vertical-align` is defined against and feeding a nested span's ascent in would trade a dy cascade for a vertical-align regression. The height error CASCADED; the baseline error is contained to one line. Original specification (superseded): emit a zero-width per-element fragment with `content_bearing: false` — both halves load-bearing, since it must not resurrect the t760 empty-wrapper rule (`<div><span></span></div>` is 0 tall in Chrome, delivered by the early return at `:7527`) nor enter the `vertical-align` metrics as an atomic would (`:7549` filters `atomic_h <= 0.0`). Idempotent where we are already right, because the fold is a `max`. ⚠⚠⚠ **The burndown lists this family under "already Chrome-correct, do NOT re-grind"** — true for the cases it was measured on (every CONTROL row here is one) and not true of this clause, which no fixture had reached: **a "do not re-grind" entry is a statement about a measurement, not about a subsystem.** It is a line-height error, therefore a **dy** error, and one wrong line box cascades down everything below it |

| pattern | where it shows up | status |
| --- | --- | --- |
| **The icon-and-label link — `<a><i class="icon"></i><span>Label</span></a>` with the wrapper at a different font-size** | every nav bar, every button row, every card footer. It is also the shape `reading_order` fires on, which the t936 sweep says blocks **9 of the 10 cheapest M1 crossings** (sites already over the shape bar, failing only jarring) | ✅ (tick 935 line-box HEIGHT · tick 939 BASELINE) — **t935 landed the height and left the text 9px too high inside it; t939 closed that, and the cause was ONE BRANCH.** `close_line` places a fragment as a real inline box *about the baseline* (`above = ascent + half_leading`) **only when it has metrics**; without them it falls to `min_h_down`, a floor that grows the line **DOWNWARD**. t935 gave the text-less wrapper its `leading` and no `(ascent, descent)`, so it took the second arm and the entire new leading landed BELOW the baseline — the line box the right HEIGHT with everything on it 9px too high, which looked like a half-fix and was in fact a fragment routed through the wrong one of two arms that were both already correct. `12px text inside a 24px wrapper: line box 36/36/36 Chrome/t935/t939, inner text below top 15/6/15`. ⚠ **The padded-edge and `<br>` spacers MUST keep the downward floor and do so BY CONSTRUCTION** — `metrics: None` leaves them on the `min_h_down` arm byte-for-byte; they hold a line open and have no baseline of their own, which is correct for a padding edge (§10.6.1: vertical padding on a non-replaced inline does not affect line height) and for a `<br>`'s reporter. The discriminator is the metrics themselves, so there is no new predicate to get wrong. Gated by `G_INLINE_BOX_LEADING` (10 claims, 5 CONTROLs); the residue row is re-pinned from OUR 6 to CHROME's 15 and RED-proven by dropping the wrapper's `metrics` (exactly the t935 state) — the height claims stay green, so **the two are separable and the gate asserts both**. ⚠ **That this moves the nine near-bar sites is a PREDICTION, not a result** — the ranking says `reading_order` blocks them and that the mechanism is upstream geometry (t871-874), not that this particular geometry is theirs; only the next sweep answers it |

| pattern | where it shows up | status |
| --- | --- | --- |
| **A geometry error that moves a box WITHOUT moving it relative to its own centre** | every displacement bug — a wrong line-box height, a wrong baseline, a wrong margin — on any element an agent might click: nav links, buttons, card titles | ⚠ NOT AN I3 EVENT (tick 945, measured) — **the agent's click point is the a11y bbox CENTRE**, so a box that translates carries its click point with it and stays perfectly clickable while being pixel-wrong. Measured on `<a><span class="big"><span>Dashboard</span></span></a>`, the exact shape t935 (line-box height, 12px) and t939 (baseline, 9px) moved: `G_CLICK_POINT` passes on the current tree, on pre-t939 AND on pre-t935 — all three states, same hour. ⚠⚠⚠ **THE RULE: a geometry error is an I3 event only when it moves a box RELATIVE TO ITS OWN CENTRE.** t851's residue qualified (an icon-wrapping `<span>` got the 4px icon's box instead of its own 17px line box — the box SHRANK around a different centre); t935/t939's does not (the box TRANSLATED). This is `g_click_point`'s own t853 lesson from the other direction — *"a rect that is right in px can still be un-clickable"* has a converse, and it is *"a rect that is WRONG in px can still be perfectly clickable"*. ⚠ Constitution checks #82 and #83 both issued the steer *"rank this as I3 and land a click-point assertion"*, three times in total, because the premise is true IN GENERAL and false for this case, and nothing had asked which. The assertion is landed anyway, as a CONTROL: it documents the finding where the next reader will look and guards the shape against a change that DOES break clickability |

| pattern | where it shows up | status |
| --- | --- | --- |
| **A list-box `<select>` — `multiple`, or `size` greater than 1** | every filter sidebar, admin form, faceted search and multi-select on the web; the CrUX tail is full of form-heavy CMS pages that use one | ⚠ MEASURED, NOT YET FIXED (tick 958) — **the model is ABSENT, not wrong.** `form_control_text` has one `"select"` arm returning the selected option's text — the dropdown, hardcoded, with no branch for `multiple` or `size`, and nothing anywhere computes a row count. Chrome-measured on a self-contained fixture: `<select multiple>` **32.4×66** vs ours **41×17**; `<select size=4>` **32.4×66** vs 41×17; `<select multiple size=2>` **32.4×34** vs 41×17. ⚠⚠⚠ **The div below six such controls sits at y=330 in Chrome and y=224 here — 106px too high**, which is the control-height `dy` cascade at full size. ⚠ **`<select multiple style="height:100px">` is EXACT at 100** — so the height machinery works and only the INTRINSIC height derived from the row count is missing, which is why the defect is small. SPEC, Chrome-measured: `rows = size if size>1, else 4 if multiple, else 1`; `height = rows × 16 + 2` (4 rows → 66, 2 rows → 34); and **no dropdown ARROW** — `native_widget_width` (`layout/lib.rs:4205`) adds 17px for a select's arrow and a list box has none, so that addition must be conditional on the same `rows == 1` test. ⚠⚠ **The `16` is Chrome's option row height and is NOT our 18px `line-height:normal` line box** — the kind of near-miss that looks right at one font size and drifts at another. ⚠ NOT specified deliberately: a list box should also RENDER ALL ITS ROWS rather than merely be tall enough for them — that is a content change, a separate and larger item, and bundling it would make the sizing fix unattributable |

| pattern | where it shows up | status |
| --- | --- | --- |
| **A TAB character in preformatted text — every `<pre>` that indents with tabs** | documentation code samples, config-file listings, diff views, anything pasted from an editor that indents with tabs; the technical pages that make up a large slice of any real corpus | ⚠ MEASURED, NOT YET FIXED (tick 959) — **a tab contributes ZERO ADVANCE, so `ab\tcd` renders as `abcd`.** Chrome-measured on shrink-to-fit `<pre>` at 16px monospace: `ab\tcd` **96.3** vs ours **31** · `tab-size:4` 57.8 vs 31 · `tab-size:0` 38.5 vs 31 · `a\tb\tc\td` **240.8 vs 31, eight times too narrow** · CONTROL `ab cd` with an ordinary space 48.2 vs 39 (so our advance is 7.8/char and all four tab rows are exactly four characters). **`tab-size` having no effect is a CONSEQUENCE** — the surface audit filed the missing property, and the property is the smaller half. Because the run is narrower its WRAP POINTS MOVE, so this is a `dy` term on text-dense pages. SPEC, one rule: **a tab advances the pen to the next multiple of (tab-size × the SPACE advance)**, tab-size defaulting to 8, and `tab-size:0` advancing nothing. ⚠⚠ **The discriminator any gate must carry**: `tab-size:4` and `tab-size:2` BOTH measure 57.8 on `ab\tcd` for different reasons — "ab" already sits at column 2, so with tab-size 2 the tab goes to column **4**, not to 2+2. A fix implementing *"add tab-size spaces"* rather than *"advance to the next stop"* gives 8 chars for tab-size:4 and 6 for tab-size:2, passing one row and failing the other. ⚠ Found while building the probe and recorded so it is not re-discovered: **`Range.getBoundingClientRect()` returns 1200 — the full viewport width — for every range**. ⚠⚠⚠ **IMPLEMENTATION CONSTRAINT (t960): the fix does NOT belong in `measure()`.** `engine/text/src/lib.rs:937` is position-independent and cached on `(font, size, text)` — which is what makes the cache sound — while a tab's advance depends on WHERE THE PEN ALREADY IS. `"ab\tcd"` and `"a\tcd"` do not differ by one character's width; the tab absorbs it. A fix that adds a width for `\t` inside `measure` is wrong for every run not starting at column 0 and would poison a cache keyed on text. Handle it where the pen exists: `collect_inline_node` splits a preserved run at each `\t` and emits the tab as its own item — exactly as it already splits `pre-wrap` at `\n` into `InlineItem::Break` — and the line pass advances to the next stop. **`InlineItem::Break` is the pattern to copy; a newline is not a character with a width either.** ⚠ This also NARROWS the blast radius: in normal `white-space` a tab collapses to a space and already measures correctly, so only PRESERVED tabs (`pre`, `pre-wrap`, `<textarea>`) are affected — not every page |
| **Container-level CSS Box Alignment — `align-content`, `justify-items` and the `place-*` shorthands** | every wrapped flex container (`display:flex` is ≥46.2% of the corpus) and every grid (18.7%): card decks and tag rows that pack their lines to the top or spread them (`align-content`), image/icon cells right- or centre-aligned inside their tracks (`justify-items`), and the one-line `place-content:center` / `place-items:center` idiom modern layouts use to centre on both axes at once | ✅ LANDED (tick 981) — the properties were **absent at all three layers**: no `ComputedStyle` field, no parse arm, no Stylo map, no `taffy_tree.rs` line, while their axis-twins `justify-content` and `align-items` were complete at all three. **Only DECLARED values were ever wrong**: `align-content`'s initial `normal` means stretch on the block axis and that is also taffy's default, so an undeclared container was Chrome-exact forever. The `place-*` shorthands each landed exactly HALF — one longhand consumed, the other discarded for want of a field — which is the cheapest instrument for telling *"arrived and dropped"* apart from *"never parsed"*. Measured Chrome-exact on 11 declared rows with 4 controls unmoved; gated by `G_CONTAINER_ALIGNMENT`. ⚠ **STILL OPEN in the same family: `grid-auto-flow: column` (and `grid-auto-rows`/`grid-auto-columns`) are unmapped** — an item that Chrome puts in the second COLUMN at x=150 lands in the third implicit ROW at y=80 |
| **Grid IMPLICIT tracks — `grid-auto-flow`, `grid-auto-rows`, `grid-auto-columns`** | every grid (`display:grid` is 18.7% of the corpus) whose items outrun its declared template, which is the normal case for any list rendered from data: card decks and product grids sized by `grid-auto-rows`, the `grid-auto-flow: column` idiom behind horizontal scrollers, timelines, calendar strips and toolbars, and `dense` in masonry-ish photo/tag walls | ✅ LANDED (tick 982) — the third property family in three ticks **absent at all three layers** while a complete twin sat beside it (`grid-template-*` was mapped; the implicit half was not). All three are fields on taffy's `Style` that nothing ever wrote, so every overflow item went into a new **row** of **content** height whatever the author said: `grid-auto-flow:column` put the 3rd item at `[0,80]` where Chrome puts it at `[150,0]`. **Only DECLARED values were ever wrong** — `row` is the initial flow and an empty auto list means `auto`, both of which are ALSO taffy's defaults, so an undeclared grid was Chrome-exact forever and no divergence sweep could rank the family. ⚠ **The first `grid-auto-rows` fixture could not fail**: in a FIXED-height container the undeclared `auto` implicit rows stretch into the free space and land exactly where declared 80px rows would — the discriminating fixture needs an **auto-height** container with zero free space, and the fixed-height version is kept as a control. ⚠ `repeat()` is legal in `grid-template-*` and forbidden in `grid-auto-*` (the list is CYCLED, not counted), which is why the two get two parsers. 11 rows Chrome-exact, 5 controls unmoved; gated by `G_GRID_IMPLICIT_TRACKS`, RED-proven four ways each confined to its own property. ⚠⚠ **STILL OPEN, and a DIFFERENT mechanism: a grid container's block size comes from its items' CONTENT, not from its resolved TRACKS** — `grid-template-rows:100px` around one 40px item is 100 tall in Chrome and **40** here, with no implicit track involved |
| **A grid container's own HEIGHT — taken from its children's bottom edge instead of its resolved row tracks** | every grid (`display:grid` is 18.7% of the corpus) with a row track that is not exactly as tall as what fills it: fixed-height hero/banner rows, `minmax()` and `fr` rows, card decks with a reserved footer row, and any template with a trailing track the data did not fill — the container is short, so everything BELOW it on the page rides up | ✅ LANDED (tick 983) — `layout_flex_or_grid` returned `max_h` (how far the lowest child reached) and **discarded the height taffy had already resolved for the container**. For FLEX the two agree — a flex line's cross size IS its tallest item — which is why it survived; for a GRID they are different questions, because **a track has a size whether or not anything fills it**. `grid-template-rows:40px 70px` holding ONE item was 40 tall against Chrome's 110; the trailing track was sized and then vanished from the container that owned it. ⚠⚠ **The fix cannot be `max(child_extent, tracks)`**: Chrome's container is sometimes SHORTER than its own content (`grid-template-rows:20px` around a 40px item is 20, and the item overflows), so a combination that only ever grows keeps that case wrong in the direction that looks safe. ⚠ **Why it hid**: a `row-gap` sits BETWEEN the children, so the lowest child's bottom already includes it — every grid with `auto` rows agreed with Chrome for the WRONG REASON. ⚠⚠ **Found by measuring the CONTAINER after every ITEM agreed** — t982's gate had 11/11 item offsets exact and two containers 40px short; an item-only readout declares a family finished one layer too early. Old-binary A/B on 4 anchor sites: mean shape 87.3%→87.4%, all four jarring invariants byte-identical — no regression AND no measurable gain, which is the honest report for a change that is Chrome-exact on the fixture and invisible on those pages. Gated by `G_GRID_CONTAINER_HEIGHT` |
| **`width: fit-content` inside a flex or grid container** | the modern shrink-to-fit idiom — badges, pills, chips, tooltips, toasts, dropdown menus, and any button or heading sized to its own label rather than to its column; Tailwind ships it as `w-fit`, and it appears almost exclusively INSIDE a flex or grid container, which is precisely where it did nothing | ✅ LANDED (tick 984) — the HALF-INSTALLED shape, not the absent one: `fit-content` parses, maps out of Stylo, and the BLOCK path consumes it in six places via `shrink_to_fit`, while the taffy path had `IntrinsicSize::FitContent => return None`. So the same declaration on the same content was right or wrong **depending on whether an ancestor happened to be `display:flex`** — a grid item kept `width:auto`, which means stretch to the track, the opposite of the request (29px word in a 200px track read 200). ⚠⚠ **It cannot be resolved to a length in the style-conversion pass**: `fit-content = min(max-content, max(min-content, stretch))` and the `stretch` term is the space the formatting context is about to hand the box — unknowable there without re-entering the measure. Expressed instead as the BOUNDS it is defined by, leaving `size.width` at `auto` so taffy's own offer supplies the middle term. ⚠⚠⚠ **THE ORDERING IS THE WHOLE SUBTLETY, and the row that catches it ALREADY PASSED before the fix**: `min-content` lives INSIDE fit-content while `max-width` clamps the RESULT, and taffy resolves min-over-max — `fit-content;max-width:20px` around a 29px word was **right by accident** (stretch to 200, clamped to 20) and the FIRST version of the fix made it 29. **A row that already passes is not a row you can leave out.** 10 rows Chrome-exact incl. 4 controls; gated by `G_FIT_CONTENT_WIDTH`, RED-proven four ways each confined. ⚠ **STILL OPEN, measured in the same battery**: a percentage `column-gap`/`row-gap` is DROPPED (`column-gap:10%` of a 300px grid is 30px in Chrome, 0 here) — `row_gap`/`column_gap` are bare `f32` px so a percentage has nowhere to be stored; the fix is a type widening to `Dim`, not a missing arm |
| **A PERCENTAGE `gap` / `row-gap` / `column-gap`** | responsive grids and flex rows that size their gutters relative to the container instead of in fixed pixels — the `gap: 2%` card-deck idiom, fluid image galleries, and any design system whose spacing scale is proportional; every such gutter silently collapsed to ZERO, closing the columns up and shifting everything to their right | ✅ LANDED (tick 985) — **not a missing arm: a FIELD THAT COULD NOT REPRESENT THE VALUE.** `row_gap`/`column_gap` were bare `f32` px, so `parse_length_px` in the minimal cascade and a `Dim::Px(p) => p, _ => 0.0` narrowing in `stylo_map` meant a percentage **arrived from Stylo intact and was thrown away one line later** — the *arrived and dropped* shape of t981's `place-*`, one type down. ⚠⚠ **A missing ARM is a hole you can grep for; a field that cannot hold the value is a hole with a PLAUSIBLE NUMBER sitting in it** — nothing was ever "unhandled", every stage did something and the something was `0.0`. Fixed by widening to `Dim`; taffy's `gap` is a `LengthPercentage` too, so the percentage crosses intact and is resolved by the participant that knows the basis. **The basis is the CONTENT box on that axis** (10% of a 300px grid is 30px; of the same grid with `padding:0 50px` it is 20px — the only row that separates content box from border box). ⚠ The AUTO-height `row-gap` case was not predicted: the basis is circular, and Chrome resolves it against the height computed with the percentage treated as ZERO (40+40=80 → gap 8); taffy agrees, so it is asserted as a fact about taffy rather than claimed as a consequence of the fix. ⚠⚠ **The CSSOM half a geometry-only gate cannot see**: `getComputedStyle().columnGap` returns `"10%"` in Chrome, and both readers formatted `{}px` around a float — a widening that fixed layout alone would print `10px`, a plausible wrong answer of the right type. 8 geometry rows + 3 CSSOM claims exact; gated by `G_PERCENTAGE_GAP`. ⚠ A 4th RED recipe (swapping the `gap` shorthand halves) **cannot fire in a page gate** — Stylo expands the shorthand first — so it lives in the `manuk-css` unit test; `gap: <row> <column>` puts the BLOCK axis first, against the `margin` analogy. ⚠ STILL OPEN: an undeclared gap serialises as `0px` where Chrome says `normal` |
| **A `transform`ed / `filter`ed ancestor as the containing block for out-of-flow descendants** | `transform` is on **34.5% of the corpus** — every animated card, carousel slide, `translateZ(0)` compositing hint and CSS-transitioned panel — and the `position:absolute`/`fixed` children inside them are the badges, close buttons, dropdown panels and tooltips a user actually clicks | ✅ LANDED (tick 986) — CSS Transforms §3 makes a transformed element the containing block for absolute AND fixed descendants **whatever its own `position`**; `filter`/`backdrop-filter` carry the same rule. `abs_containing_block` tested only `position != Static` and `fixed` was handed the viewport unconditionally, so an out-of-flow box inside a transformed wrapper escaped past it: measured `[10, -1328]` against Chrome's `[20, 20]`. ⚠⚠⚠ **Not a rounding error — a box on a different part of the page**, so this is an I3/jarring-class defect, not a shape one. ⚠⚠ **The `absolute` row shows the old test was the WRONG TEST**: its wrapper is `position:static`, so `position != Static` made an ancestor that was *right there* invisible — the code asked a question, got a truthful answer, and the question was wrong. That is a different failure mode from "unimplemented" and it greps as covered. ⚠ **One predicate, not a `transform` special case**: `filter: blur(0px)` — a NO-OP blur — is enough, measured; narrowing to `transform` fails exactly the filter row. 6 rows Chrome-exact incl. 2 controls; gated by `G_TRANSFORM_CONTAINING_BLOCK`, RED-proven three ways with the absolute and fixed halves separately provable. Old-binary A/B on 4 anchor sites: byte-identical (87.4% mean shape, all four jarring invariants unchanged). ⚠⚠ **STILL OPEN, the t985 shape one level up**: `will-change` / `contain` / `perspective` obey the same rule and have **no ComputedStyle field at all** — not unhandled values, values with nowhere to live. `will-change:transform` (the standard compositing hint) reads `[20,-364]` against Chrome's `[20,20]` |
| **`will-change` / `contain` / `perspective` as containing-block creators** | `will-change: transform` is *the* standard compositing hint and appears on any page with a scroll-linked or animated panel; `contain: layout|paint|content` is the standard perf hint on virtualised lists and card grids. Both wrap exactly the components whose dropdowns, tooltips and badges are `position:absolute` | ✅ LANDED (tick 987) — t986's named residue, and the same "nowhere to live" shape as t985's `gap`: these three had **no `ComputedStyle` field at all**, so the fix was a cascade addition, not a layout one. They now reach layout as ONE `bool` (all layout needs; a per-node `will-change` string list is the allocation the custom-property field documents as a measured mistake). ⚠⚠⚠ **THE NEGATIVE HALF IS THE WHOLE DIFFICULTY**: a predicate written as `!will_change.is_empty() \|\| !contain.is_empty()` — the obvious version — **passes all ten positive rows and is wrong about all three negatives**. `will-change: opacity` creates a STACKING CONTEXT (a different thing the same property also does) and NOT a containing block; `contain: style` and `contain: size` are containment of other kinds. All four negative rows Chrome-measured, not reasoned from the grammar, and the naive predicate is the most useful RED recipe in the gate. ⚠⚠ On the Stylo path the keyword list is NOT re-derived: `WillChangeBits::FIXPOS_CB_NON_SVG` is literally "a property that creates a containing block for fixed-position descendants will change" — ask the engine that already computed the answer; re-deriving by hand is exactly how the `opacity` case ships wrong. 14 rows Chrome-exact incl. 4 negative controls; gated by `G_TRANSFORM_CONTAINING_BLOCK` + `will_change_contain_and_perspective_create_a_containing_block_only_for_the_right_values`. ⚠ Cost of the boolean, recorded so it is not rediscovered: `getComputedStyle().willChange` cannot be served from it |
| **`text-indent` — the first line's start edge** | every article, blog and documentation page that indents paragraphs, every `text-indent:-9999px` image-replacement logo still in the wild, and — because the defect moved WRAP POINTS — any text-dense page that sets it at all, where a changed break re-flows the whole block below | ✅ LANDED (tick 988) — the indent was reduced from `line_avail` AND added to the first fragment's pen while `line_left` never moved, so the wrap test `pen + space + advance > line_avail` **charged it twice**, and alignment used a band whose left edge was wrong. Chrome breaks `aa bb / cc` where we broke `aa / bb cc`. ⚠⚠⚠ **A WRAP-POINT ERROR THAT DOES NOT CHANGE THE LINE COUNT IS INVISIBLE TO HEIGHT, WIDTH AND TEXT CONTENT AT ONCE** — two lines either way, every character present, container identical. The 20-row text battery could see it only as a 20px difference in an inline box's fragment UNION; a marker on the SECOND line is what named it. ⚠⚠ **The `text-align` rows are the evidence the MODEL is right, not a bonus**: "the indent is a leading space" and "the start edge moves in, the end edge does not" explain the break symptom equally well and disagree by 10px under `text-align:center` (186 vs Chrome's 196) and 20px under `right` (351 vs 371). **Two models that agree on the symptom you noticed are separated by the property you did not think to combine it with.** ⚠ A negative indent still widens the line (`-9999px` image replacement) because both terms flip sign together; clamping at zero "to be safe" fails that control and nothing else. 9 rows Chrome-exact incl. 6 controls; gated by `G_TEXT_INDENT_EDGES`, RED-proven three ways. ⚠ A fourth recipe (indent on EVERY line) does NOT fire — wrapped lines never reach the `first_line` test, because the break path sets `line_left`/`line_avail` directly |
| **`vertical-align` on a TABLE CELL** | the pre-flexbox vertical-centring idiom, still everywhere: toolbars, data grids, icon+label rows, and the whole `display:table` / `display:table-cell` centring pattern that legacy CSS reaches for; also every real `<table>` whose rows have mixed-height cells | ✅ LANDED (tick 989) — a cell is laid out at its own content height and then STRETCHED to the row, and **stretching a box does not move what is inside it**: the stretch is one assignment to `rect.height`, so every cell was top-aligned whatever the property said (19px word in a 60px cell at y=2 against Chrome's 20 for `middle` / 38 for `bottom`). ⚠⚠⚠ **THE TRAP COST A BUILD**: the obvious implementation reads the free space as `row_height − cell_box_height`, which is **ZERO for exactly the cells that have the most free space** — a cell with `height:60px` around a 24px line reports a *border-box* height of 60 because the explicit height was already applied. **`height` on a box you have already sized tells you what you ASKED FOR, not what is inside it.** The free space must come from the cell's NATURAL content height, now returned by `layout_cell`. ⚠⚠ **The RED proof for "shift the content, not the cell" came back GREEN** — every row in the first draft measured a `<span>` inside the cell, which moves under either rule; the control that separates them (the CELL BOX of a cell that actually shifts) had to be added after the RED refused to fire. **A gate whose rows all measure the same thing cannot tell two rules apart, and the RED proof reveals that — not the passing run.** ⚠ `baseline` (the CSS initial value for a cell) is approximated as `top` and that approximation is NAMED rather than shipped silently. Gated by `G_TABLE_CELL_VALIGN`. ⚠⚠ **STILL OPEN, from the same 16-row battery — 5 divergences in 4 mechanisms**: ROWSPAN row-height distribution (a 60px rowspan=2 must give 30/30, we give 24/36 with all the overflow on the LAST row — this is VI.2's "t933 row-height distribution", now with a number), CAPTION (reserves no space and does not widen the table), and THEAD ORDERING (a `<thead>` written after a `<tbody>` must still render FIRST) |
| **`rowspan` row-height distribution** | every real data table with a merged vertical cell — invoice line-item blocks, schedules and timetables with merged slots, spec tables with a grouped first column, comparison matrices; the error grows everything below the span, so it displaces a whole neighbourhood rather than one cell | ✅ LANDED (tick 990) — **the defect `CONSTITUTION.MD` VI.2 has carried as "t933 row-height distribution" since check #82**, and t989's 16-row table battery is the first fixture to put numbers on it. A rowspan cell taller than the rows it covers had its whole excess added to the **LAST** spanned row (`row_h[last] += *bh - spanned`); Chrome shares it across every spanned row **proportionally to their natural heights**. A 100px rowspan=2 over rows of 40 and 24 is 63/38 in Chrome and was 40/84 here. ⚠⚠⚠ **PROPORTIONAL vs EVEN is decided by exactly ONE row of the fixture**: 36px of excess over 40/24 splits 22.5/13.5 → 63/38, where an even split gives 58/42 — and every *equal*-rows case degenerates to the same answer under both rules, so a fixture of equal rows ships either one. **The rows that discriminate are rarely the rows that made you look** (the battery that found this used equal rows). Even distribution survives only as the zero-height fallback, where "proportional" has no meaning. 11 rows Chrome-exact incl. 2 controls; gated by `G_ROWSPAN_DISTRIBUTION`, RED-proven three ways each confined — notably, dropping the `deficit > 0` guard SHRINKS rows a shorter rowspan cell has no right to shrink (18.75/11.25). ⚠ Still open from the same battery: `<caption>` reserves no space and does not widen the table; a `<thead>` written after a `<tbody>` renders in source order instead of first |
| **Table row-group ORDER — `<thead>` / `<tbody>` / `<tfoot>`** | the classic HTML4 idiom puts `<tfoot>` BEFORE `<tbody>` (so a long table's footer reaches the parser before its thousand body rows) and it is still everywhere in legacy markup, invoice templates and report generators — every one of which rendered its totals row at the TOP of the table | ✅ LANDED (tick 991) — CSS Tables lays row groups out **header → body → footer regardless of source order**, and our UA sheet said `thead, tbody, tfoot { display: table-row-group }`: one value for three groups, **discarding the only thing that distinguishes them**. ⚠⚠⚠ **A totals row at the top of an invoice is a READING-ORDER error, not a geometry one** — every number present, correctly sized, and meaning something else. I3 class, not shape class, and a `dy`-ranked burndown cannot see the difference. ⚠⚠ **THE FIX IS NOT WHERE THE SYMPTOM IS**: the layout code walked the DOM in order and adding a rank there could not work, because every group arrived carrying the same `display` value. The distinction is made in the UA SHEET (Chrome makes it there too); two new `Display` variants exist so the value can SURVIVE the cascade. **A fold that discards a distinction reads as a simplification right up until something needs the distinction.** The two RED recipes prove the pipe from both ends — restoring the folded UA rule fails with the layout rank intact (**the layout fix alone is inert**), dropping the sort fails with the UA distinction intact. ⚠ The sort must be STABLE (two `<tbody>`s keep document order); swapping in `sort_unstable_by_key` does NOT go red today with two elements, recorded as a NON-red rather than claimed. 12 rows Chrome-exact incl. 2 controls; gated by `G_ROW_GROUP_ORDER`. With this the t989 table battery is 3 of 4 mechanisms closed — only `<caption>` remains |
| **`<table><caption>`** | data tables in documentation, reference material, government and academic pages, and anything generated by a CMS table widget — where the caption is the table's accessible NAME, so dropping it costs both the visible label and the a11y/agent surface | ✅ LANDED (tick 992) — the caption was skipped alongside the column groups and never laid out: **three defects in one dropped child**, of two different kinds. The text did not appear (a MISSING_BOX, not a geometry error), the rows did not move down for it, and the table did not widen for it. ⚠⚠⚠ **A CAPTION WIDENS ITS TABLE, TO ITS MIN-CONTENT WIDTH** — a one-cell table holding `x` (10px) with the caption `a very long caption` is **67** wide in Chrome (the longest word) and the column takes the extra, distributed exactly as t990 distributes a rowspan's surplus over its rows, on the other axis. **Min-content and NOT max-content because the caption may WRAP**: at 67 it is three lines tall and Chrome keeps it three lines; a max-content floor gives 183 and one line — a wrong answer of the right shape, and **the row that distinguishes them is the caption's HEIGHT, not its width**. ⚠⚠ The caption is emitted FIRST among the table's children because it reads first in the semantic order the agent surface walks — a table's accessible name after its data is an I3 defect; ⚠ reordering it does NOT fail the gate (whose assertions are geometric), recorded as a NON-red for an a11y-order gate. ⚠ `caption-side: bottom` is Chrome-measured and NOT built — no `caption_side` field on `ComputedStyle`, the "nowhere to live" shape again. Gated by `G_TABLE_CAPTION`. **With this the whole 16-row table battery of t989 is Chrome-exact: four mechanisms found, four closed** |
| **`border-collapse: collapse` — shared-edge geometry** | every CSS reset sets it and nearly every real data table uses it: documentation tables, spec tables, admin grids, CMS-generated tables. The error is cumulative — we give every cell its full border on all four sides and share nothing, so a collapsed table is `(n+1) × border` too wide and every column after the first is displaced further than the last | ⚠ MEASURED, NOT YET FIXED (tick 993) — **the rule, derived from Chrome: each edge's collapsed width is the `max` of the borders meeting at it, and each cell's border box extends HALF of that per side.** Two uniform 10px cells: Chrome `[5,5,20,34]` and `[25,5,20,34]`, ours `[0,0,30,44]` and `[30,0,30,44]`. The UNEQUAL case is what proves max-then-halve: a 4px cell beside a 20px cell gives `[2,10,22,44]` / `[24,10,30,44]` — shared edge `max(4,20)=20`, half 10, so cell 1 is `2+10+10=22` wide. A table border collapses with the outer cell edges the same way (`max(10,2)=10`). ⚠ The SEPARATE model is already exact (control). ⚠ The full algorithm also has conflict resolution (style priority, cell vs row vs table) these rows do not exercise. ⚠⚠ **Chrome's own collapsed-table OUTER width has a 1px quirk** (49 where the arithmetic says 50, 63 where it says 64) — **a gate must assert the CELLS, not the table box** |
| **An inline element with a frame on the INLINE AXIS ONLY** | `<code>`, `<kbd>`, padded `<a>` chips and pills, syntax-highlighted spans, badge spans — the horizontal-only frame is the overwhelmingly common inline decoration, and it is exactly the shape that breaks | ✅ LANDED (tick 994; measured t993) — t851 established that a non-replaced inline's box is its own content area **resolved PER AXIS**, and that holds for a plain inline and for one with an ALL-SIDES border (`border:12px` reads `[10,-10,34,43]`, exact). But `border-left:12px` and `padding-left:12px` both read `[10,0,22,21]` against Chrome's `[10,2,22,19]`: **the inline axis is right in every row** (the frame advances the pen correctly) and only the BLOCK axis goes wrong, and only when the VERTICAL frame is zero — the box top becomes the LINE BOX's top rather than the content area's, 2px of half-leading high and 2px over in height. A narrow path, not the general case, which is why t851's fix did not cover it. **THE CAUSE**: `collect_inline_node` computed the vertical report behind `if pad_t > 0.0 \|\| pad_b > 0.0`, so a horizontal-only frame emitted an edge spacer carrying NO vertical report; the spacer fell back to the LINE BOX, and an inline's box is the UNION of its fragments — line-box spacer (0..21) ∪ own word (2..21) = 0..21. ⚠⚠⚠ **A CONDITIONAL THAT GUARDS A COMPUTATION BY THE AXIS IT HAPPENS TO BE ABOUT IS HOW A PER-AXIS RULE LOSES ONE AXIS** — the report was written FOR vertical padding so it was gated ON vertical padding, and with zero vertical padding its terms simply add nothing, which is the right answer for ANY framed inline. Fixed by widening the condition to any frame. 5 rows Chrome-exact incl. 3 controls; gated by `G_INLINE_FRAME_BOX`, RED-proven two ways; ⚠ dropping `pad_r` does NOT go red (every framed row has a LEFT edge) — that half is reasoned, not measured. Old-binary A/B byte-identical |
| **Form-control and replaced-element metrics — `<input>` · checkbox/radio · `<select>` · `<textarea>` · `<fieldset>`** | the highest-frequency area in the whole corpus: `<button>` 55.6%, `<input>` 51.5%, `<svg>` 34.5%. Every login form, search box, filter panel, settings page and checkout | ⚠ MEASURED, NOT YET FIXED (tick 995) — 19-row battery, **10 exact and 9 diverging in FIVE mechanisms**. ✅ **`<button>` is exact on all four of its rows** — t972's 2px-outset UA border holds, including as a flex item and for its label's centring. Diverging: `<input>` 2px short and 3px high on all three rows *with its WIDTH exact* (**which rules out the symmetric-border shape t972 fixed — assuming it was that is the obvious wrong move**); checkbox/radio 15×15 against Chrome's 13×13; `<select>` 3px narrow AND 2px short; `<textarea>` 2px too tall with width exact; `<fieldset>` **24 against 50** — rendered as a plain block with NO UA border or padding, its `<legend>` a second mechanism inside the first. ⚠⚠ **Chrome's UA computed values do NOT reconcile with its own measured boxes** (`<input>`: content 15 + padding 2 + border 4 = 21, measures 19; width 197+8 = 205, measures 201) — **a native control's USED border is the platform THEME's, not the UA sheet's**, so a fix driven from `getComputedStyle` alone is built on numbers Chrome does not lay out with. The full UA table is banked in `docs/wiki/box-layout.md`. ⚠ Also exact and worth not re-checking: `<svg>` with w/h attrs, `<svg>` with only a viewBox (a RATIO never a size — it fills 400×200), `<img>` with w/h and no src, `<label>` as an ordinary inline |
