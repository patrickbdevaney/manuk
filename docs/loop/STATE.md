...
| Axis | Score | Notes |
|------|------|-------|
| RENDER | 79 | Stylo cascade, flex/grid (**incl. flex/grid items with block children — a major collapse bug is fixed**), grid-template-areas, UAX#14 breaking, position:sticky, responsive `@media`. **VISUAL verification now autonomous** (`manuk-wpt render` PNG + Chrome ref). **border-radius + box-shadow now paint.** Missing paint: per-corner/elliptical radii, inset/multiple shadows. Container queries + Wikipedia collapsed-menu JS still partial. |
...
block-child collapse fix (Tick 13)**; **border-radius + box-shadow paint (Tick 14)**.
