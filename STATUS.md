...
```
TICK:              803
LAST_AUDIT_TICK:   798          (self-audit due every 10 ticks — the hook BLOCKS commits past that)
LAST_SURFACE_AUDIT: 798         (surface audit due every 10 ticks — from docs/loop/SURFACE-AUDIT.md)
LAST_CONSTITUTION_CHECK: 796     (constitution re-read due every 8 ticks — from docs/loop/CONSTITUTION-CHECK.md; anchors the loop to CONSTITUTION.MD)
LOOP_BUDGET:       4197 ticks remaining (target tick 5000) — from docs/loop/AUTOLOOP; the loop STOPS and reports at 0
LAST_WALL_AUDIT:   796         (wall-time audit due every 20 ticks — scripts/wall-audit.sh; hunts wall bloat without cutting a gate)
