...
printf "  %s▶ FINISH-LINE STATUS (observer, tick 208): fetch-STREAMING (ReadableStream+SSE+progressive-XHR, all gated), a11y-STATES, WebSocket (transport+page-facing), and scroll-ANCHORING are ALL LANDED. %sforced-reflow is the ONLY remaining finish-line lever%s (getBoundingClientRect/ResizeObserver returning live geometry after a mid-tick DOM mutation; constellation row still 'partial'). LAND IT NEXT to complete Phase 0 — the actuation-completers (hover/dblclick/contextmenu/file-input/drag dispatchers, click-activation) are valuable but come AFTER the finish line. When forced-reflow lands, Phase 0 is complete.%s\n" "$C$B" "$R$B" "$O" "$O"
