...
printf "  %s\U0001F3C1 PHASE-0 FINISH LINE (USER DIRECTIVE, tick 195) — the FIVE remaining Phase-0 levers; work top-down, each a GENERALIZED primitive: (1) fetch STREAMING response body — bridge manuk_net::fetch_streaming to a JS ReadableStream + real SSE + readyState-3 XHR; AI-chat answers (claude.ai/ChatGPT/Gemini/Grok) render + console live-logs work [HIGHEST VALUE, HALF-BUILT]. (2) a11y node STATES — add checked/expanded/selected/disabled/value/focused to A11yNode; the agent can CONFIRM ITS OWN ACTIONS [the agentic moat, bounded a11y/src/lib.rs]. (3) WebSocket via tokio-tungstenite [BORROW the crate — live chat/DMs/presence + cloud live-logs]. (4) scroll-anchoring/overflow-anchor [feeds stop jumping on load-more]. (5) forced-reflow for getBoundingClientRect/ResizeObserver mid-tick [virtualized lists render correctly]. When these FIVE land on the existing base, Phase 0 is DECLARED good-enough: touch .git/manuk-phase0-complete + JOURNAL note (triggers the AGENTIC-PHASES-PLAN Phase-1 cascade).%s\n" "$R$B" "$O"
printf "  %s▶ FINISH-LINE — DO forced-reflow NOW (observer, tick 211): 4/5 landed (streaming, a11y-states, WebSocket, scroll-anchoring). forced-reflow is the ONLY remaining finish-line lever and IT IS BOUNDED, NOT architectural — the relayout machinery ALREADY EXISTS: page::relayout_incremental + RestyleDamage (the code already does 'appending a box forces reflow' and runs scripts against a snapshot 'so getBoundingClientRect works'). The ONLY missing piece: wire it into the READ PATH — when JS reads getBoundingClientRect / ResizeObserver and the DOM is DIRTY since the pre-script snapshot, call relayout_incremental FIRST, then return fresh rects (the react-window/virtuoso measure->mutate->measure case). This is one bounded tick. PARK the actuation-completers (label/disabled/submit/hover/dblclick — valuable but AFTER Phase 0). Do forced-reflow NEXT; when it lands, Phase 0 is COMPLETE.%s\n" "$R$B" "$O"
