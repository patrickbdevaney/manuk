...
printf "  %s\U0001F3C1 PHASE-0 FINISH LINE (USER DIRECTIVE, tick 195): the remaining Phase-0 levers, IN PRIORITY ORDER, are (1) WebSocket via tokio-tungstenite (row 53 — live chat/DMs/presence + cloud live-logs; BORROW the crate, do not build), (2) scroll-anchoring / overflow-anchor (row 108 — feeds stop jumping on load-more), (3) forced-reflow for getBoundingClientRect/ResizeObserver mid-tick (row 109 — virtualized lists render correctly). WORK THESE NEXT, top-down. Each is a generalized primitive (all real-time / all feeds / all virtualized lists). When these land on top of the existing base, Phase 0 is DECLARED good-enough per the user (touch .git/manuk-phase0-complete + JOURNAL note).%s\n" "$R$B" "$O"
