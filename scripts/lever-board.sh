...
printf "  %s▶ DO NEXT (user directive, tick 222): MEDIA/YouTube + PROBE-UNKNOWNS — NOT another CSS-render tick. MEDIA is a subsystem; START it as BOUNDED ticks in this order (borrow, feature-gate each, add a falsifiable gate — a sample stream buffers/plays, not a WPT count): [M1] MSE SKELETON — JS MediaSource/SourceBuffer/isTypeSupported/URL.createObjectURL(ms)/addSourceBuffer/appendBuffer that ACCEPT+QUEUE bytes and fire sourceopen/updateend/.buffered/endOfStream (NO decode yet — bounded, do this FIRST). [M2] arraybuffer/Range binary body if not already from streaming. [M3] symphonia demux fMP4+webm (feature media). [M4] AAC decode (symphonia) + cpal audio out = master clock. [M5] VP9 decode (libvpx) OR AV1 (rav1d) -> DecodedImage into the <video> node. [M6] playback engine + A/V sync (currentTime/seek/timeupdate/playing/ended). [M7] controls+buffering+fullscreen+WebVTT. Steer YouTube to VP9+AAC via isTypeSupported; no ffmpeg; DRM out. IN PARALLEL, cheap wins: PROBE an UNKNOWN each time media needs a build/think-gap (constellation --gaps '?' rows — hydration-style measure-probes flip cells green cheaply). %sDo M1 or a probe on the very next tick.%s\n" "$Y$B" "$R$B" "$O"
printf "  %s▶ ALSO CO-CRITICAL: OAUTH LOGIN (edge sweep t222) — you cannot daily-drive the LOGGED-IN web without it; treat as co-#1 with media. Google v3/GIS + compatible (Auth0/Clerk/Okta/Sign-in-with-X), decomposed: [O1] PROBE+build the OAuth 2.0 REDIRECT flow e2e (navigate to IdP -> auth -> redirect back with code/token -> app session cookie; POST+cookies+redirect already partial — wire+gate the full handshake, likeliest path). [O2] interactive cross-origin IFRAME RE-RENDER (row 50 gap — the consent/One-Tap iframe is a frozen snapshot; make it re-render on mutation; also fixes 3-D-Secure). [O3] popup flow: window.open -> accounts.google -> postMessage(token) to opener (partial — verify e2e). [O4] third-party/cross-site cookies for the accounts.google->app SSO handoff. [O5] FedCM navigator.credentials.get (the NEWEST GIS One-Tap default — currently MISSING). Gate each with a real IdP round-trip.%s\n" "$R$B" "$O"
