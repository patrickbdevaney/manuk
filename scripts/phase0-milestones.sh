...
if [ "$MODE" = "--oneline" ]; then
  case "$CUR" in
    M1) printf "PHASE-0 ▸ M1 RENDER(corpus=%s) %s%%→95%% (need +%s, %s)%s  [M2 function pending]\n" "${M1_CORPUS}" "${M1_PASS:-?}" "$NEED" "$SLOPE_TXT" "$CORPUS_NOTE" ;;
...
printf "  %sM1%s RENDER  shape>=0.75 on >=95%% of in-scope representative CrUX   %s\n" "$B" "$O" "$(_st $M1_DONE M1)"
printf "       now %s%s%%%s on corpus=%s  (%s/%s in-scope · target %s · need +%s)   %s%s\n" "$B" "${M1_PASS:-?}" "$O" "${M1_CORPUS}" "${M1_GE75:-?}" "${M1_INSCOPE:-?}" "$TARGET" "$NEED" "$SLOPE_TXT" "$CORPUS_NOTE"
