...
// ─────────────────────────────────────────────────────────────────────────────────────────────────
// SELECTOR-PATH KEYING — moved here from `main.rs` at t1332.
//
// ⚠ It lived in the BINARY, so the library's own gates could not reach it and
// `G_PATH_KEY_CONTRACT` could not compare our walker against the reference probe's without
// duplicating it — which is exactly the drift the contract exists to prevent. One definition, in
// the library, reachable by both.
// ─────────────────────────────────────────────────────────────────────────────────────────────────
...
pub fn sig_of(dom: &manuk_dom::Dom, n: manuk_dom::NodeId) -> String {
...
/// not.
pub fn path_of(dom: &manuk_dom::Dom, n: manuk_dom::NodeId) -> Option<String> {
    ...
    loop {
        ...
        // ⚠⚠⚠ **THE COUNTER CANNOT INCLUDE THE SIG, AND t1332 PROVED IT BY TRYING.**
        //
        // The sibling counter is sig-BLIND, and that looks like a defect: one `<div>` only one
        // engine creates shifts every later `div:nth-of-type(n)`, so each shifted element keys
        // differently and scores as MISSING. On `www.crazyshop.pl` (two JS-injected
        // `div.siiimpleToast` at the top of `<body>`) that is real and large — `135 missing` fell to
        // **23** when the counter was made sig-aware.
        //
        // ⚠ **And the rest of the instrument fell with it.** `strip_sigs` removes every `.SIG` from
        // both sides' keys BEFORE the comparison (`main.rs`'s fidelity path and `run_oracle_merge`
        // alike), deliberately — so a class the two engines disagree about does not unmatch an
        // element. With a sig-AWARE counter, stripping then maps distinct siblings onto the SAME
        // key and they overwrite each other:
        //
        // ```text
        ...
        // ```
        //
        // ⭐ **The counter and the comparison are COUPLED**: match sig-blind and count sig-blind
        // (this), or match with sigs and count with sigs (a different instrument, fragile to any
        // class the two engines disagree about — which is what `strip_sigs` exists to absorb). The
        // middle is incoherent, and the middle is what a reading of the key alone suggests.
        let mut i = 1usize;
