...
impl Ctx<'_> {
    ...
    fn layout_children(
        ...
        for &k in &kids {
            ...
            if self.kid_is_float(k) {
                ...
                //    earlier in the source document."* That is a bound on the line's **top**, and we
                //    flushed the pending run — which advances `cur_y` past the whole run — and then
                //    placed the float at the bottom. Every such float came out **one line-height too
                //    low**, and it drags its entire exclusion band with it.
                //
                //    `<img class="alignright">` inside a paragraph is exactly this shape, and it is
                //    the most common float on the content web. Measured:
                //
                //    ```text
                //                                                       Chrome   before   after
                //      float FIRST, then text                             0        0        0
                //      text, then the float (one line)                    0       20        0
                //      a <p> block, then the float                       52       52       52
                //      text wrapping to TWO lines, then the float        20       40       20
                //      line-height: 40px, text then float                 0       40        0
                //      text<br>second, then the float                    20       40       20
                //    ```
                //
                //    ⚠ It is the **LAST** line's top and not the run's, which the two-line rows are
                //    what prove: a run that wraps puts the float on the line it is actually in.
                //    Fragments on one line share a `line_top`, so the maximum over the fragments the
                //    flush just produced IS that line — no new plumbing, and no signature change to
                //    a function with five other callers.
                //
                //    ⚠ `place()` still scans downward from here, so a float that does not FIT beside
                //    what is already on that line drops to the next band exactly as before — this
                //    moves the starting point, not the search.
                //    ⚠⚠⚠ **AND THE FLOAT ONLY JOINS THAT LINE IF IT STILL FITS ON IT** — the other
                //    half of the rule, and the row that found it is a 280px float after text in a
                //    300px block: Chrome puts it on the NEXT line (y=40) and a top-bound alone puts
                //    it on the current one. `place()` cannot see this for us: it scans bands of
                //    FLOATS, and what is in the way here is the line's own already-placed inline
                ...
                let last_line = boxes[before..].iter().find_map(|b| match &b.content {
                    BoxContent::Inline(frags) if !frags.is_empty() => {
                        let top = frags
                            .iter()
                            .fold(f32::NEG_INFINITY, |m, f| m.max(f.line_top));
                        let used = frags
                            .iter()
                            .filter(|f| (f.line_top - top).abs() < 0.5)
                            .fold(cx, |m, f| m.max(f.x + f.width))
                            - cx;
                        Some((top, used))
                    }
                    _ => None,
                ...
                // added one measured defect at a time). A third copy here would drift from it the
    ...
    fn layout_float(
        ...
        cb_left: f32,
        // `(line_top, remaining)` for the line box this float was written into, when it follows
        // inline content — CSS 2.1 §9.5 rule 6. It is a CANDIDATE, not an instruction: the float
        ...
        // ── ⚠⚠⚠ **RULE 6 IS A BOUND ON THE LINE'S TOP, AND WE USED ITS BOTTOM.** CSS 2.1 §9.5:
        // *"the outer top of a floating box may not be higher than the top of any line-box
        // containing a box generated by an element earlier in the source document."* A float written
        // after text on a line belongs at that line's TOP; we flushed the pending inline run — which
        // advances past the whole run — and placed the float below it, **one line-height too low**,
        // dragging its entire exclusion band down with it. `<img class="alignright">` inside a
        // paragraph is exactly this shape, and it is the most common float on the content web.
        //
        // ⚠⚠ **The float only joins that line if it still FITS on it**, and the row that found that
        // is a 280px float after text in a 300px block: Chrome puts it on the NEXT line. `place()`
        // cannot decide this for us — it scans bands of FLOATS, and what is in the way here is the
        // line's own already-placed inline content, which is not a float and is not in the context.
        // That is also why the test is here rather than at the call site: `width` is only final now.
        //
        // ⚠ `clear_to` is re-applied to the candidate. `top` was already cleared above, and joining a
        // line must never lift a float above something it was told to clear — a `clear: left` float
        // in a paragraph would otherwise jump back up beside the very float it is clearing.
        let top = match line_fit {
            Some((line_top, remaining)) if width + non_content <= remaining => {
                floats.clear_to(s.clear, line_top)
            }
            _ => top,
        };
