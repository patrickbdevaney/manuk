//! **G_WEBFONT_RELAYOUT — the web font downloaded, decoded, registered and resolved, and the document
//! was never re-laid-out with it.**
//!
...
#[test]
fn a_downloaded_webfont_is_actually_laid_out_with() {
    let fonts = FontContext::new();
    let origin = font_origin();

    // The `@font-face` is INLINE — no external stylesheet — which is the case the relayout guard
    // missed, and the case every critical-CSS build produces.
    let html = format!(
        r#"<!doctype html><html><head><style>
             @font-face {{ font-family: 'AhemTest'; src: url({origin}/ahem.woff2) format('woff2'); }}
    ...
    assert!(
        (ahem - 100.0).abs() < 0.5,
        "G_WEBFONT_RELAYOUT: text in a downloaded @font-face family measured {ahem}px, not 100px.\n\n  \
    ...
    assert!(
        ...
        "G_WEBFONT_RELAYOUT is VACUOUS: the serif fallback also measured {fallback}px, so `== 100` \
