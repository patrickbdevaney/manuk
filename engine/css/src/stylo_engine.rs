...
/// **The candidate property names the supported-set registry asks about.**
///
/// ⚠ **A CANDIDATE list, not the answer** — the answer is whatever [`supports_condition`] says about
/// each of these, which is the engine's own oracle and the same one `@supports` and `CSS.supports()`
/// consult. A name absent from this list is simply never asked, so the registry is a **lower bound**
/// on what the engine supports, and adding a name here can only ever make it more complete.
///
/// **Its bound, stated rather than implied:** this is the common CSS surface — shorthands and
/// longhands a real sheet writes — not every property in every module. It is deliberately not
/// derived from `ComputedStyle`'s fields, because those are what the engine MODELS and the question
/// here is what it can PARSE, which is a larger and different set (`PARSE_ONLY_LONGHANDS` and its
/// documented opposite exist precisely because those two sets disagree).
...
/// **The set of CSS property names this engine supports** — the enumerable registry
/// `CSSStyleDeclaration` needs and did not have.
///
/// CSSOM says a declaration exposes an IDL attribute for **every supported property**, set or not.
/// `el.style`'s proxy answered `has` with *"is this property currently SET"*, so
/// `'gridTemplateColumns' in el.style` — **the** CSS feature-detection idiom, the one Modernizr and
/// every polyfill loader is built on — was `false` for every feature this engine actually has. A
/// capability we possess reported as absent makes pages take their fallback path against a working
/// engine, which is the inverse of the usual failure and strictly worse.
///
/// Tick 1171 named the fix and its blocker: *"`supports_condition` answers one declaration at a time
/// and is not enumerable, so there is no list to hand the proxy."* The half that was missing is that
/// **one-at-a-time is fine if you have candidates to ask** — [`CANDIDATE_PROPERTIES`] is that list,
/// and asking all of them costs **21ms**, measured, paid once and lazily.
pub fn supported_property_names() -> &'static [&'static str] {
    static SET: std::sync::OnceLock<Vec<&'static str>> = std::sync::OnceLock::new();
    SET.get_or_init(|| {
        CANDIDATE_PROPERTIES
            .iter()
            .copied()
            .filter(|p| supports_condition(&format!("{p}: initial")))
            .collect()
    })
}
